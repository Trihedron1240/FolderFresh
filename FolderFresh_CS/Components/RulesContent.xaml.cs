...
public sealed partial class RulesContent : UserControl
{
    ...
    private async Task LoadSettingsAsync()
    {
        ...
        {
            var settings = await _settingsService.LoadSettingsAsync();
            FallbackToggle.IsOn = settings.FallbackToCategories;
        }
        finally
        {
    ...
    private async void FallbackToggle_Toggled(object sender, RoutedEventArgs e)
    {
        // Don't save during initial load
        if (_isLoadingSettings) return;

        var settings = await _settingsService.LoadSettingsAsync();
        settings.FallbackToCategories = FallbackToggle.IsOn;
